...
public sealed class ProxyHeadersOptions : IValidatableObject
{
	...
	/// <summary>
	/// Gets or sets the mode for processing forwarded headers.
	/// Defaults to <see cref="ForwardedHeaderMode.Disabled"/> for security (secure-by-default).
	/// </summary>
	/// <remarks>
	///     <para>
	///     <see cref="ForwardedHeaderMode.Disabled"/>: No header processing (direct access, no proxy).
	///     </para>
	///     <para>
	///     <see cref="ForwardedHeaderMode.Cloud"/>: Trust all headers (for Azure, AWS, GCP managed proxies).
	///     </para>
	///     <para>
	///     <see cref="ForwardedHeaderMode.SelfManaged"/>: Only trust explicitly configured proxies/networks.
	///     </para>
	/// </remarks>
	...
	/// </summary>
	/// <remarks>
	/// If the value is <see langword="null"/>, a conservative default of <c>1</c> is used, which matches the
	/// common scenario of a single reverse proxy or load balancer in front of the application.
	/// Startup validation will fail if the value is less than 1.
	/// </remarks>
	[Range(1, int.MaxValue, ErrorMessage = "ForwardLimit must be greater than or equal to 1 when specified.")]
	public int? ForwardLimit { get; set; }

	/// <summary>
	/// Gets or sets the list of proxy IP addresses that are considered trusted.
	/// Only relevant in <see cref="ForwardedHeaderMode.SelfManaged"/> mode.
	/// </summary>
	/// <remarks>
	///     <para>
	///     <strong>SelfManaged mode:</strong> Must contain at least one entry (this list OR <see cref="TrustedNetworks"/>).
	///     Startup validation will fail if both lists are empty.
	///     </para>
	///     <para>
	///     <strong>Cloud mode:</strong> This list is ignored (cloud platforms control headers).
	///     </para>
	///     <para>
	///     <strong>Disabled mode:</strong> This list is ignored (no header processing).
	///     </para>
	///     <para>
	///     Example: <c>["10.0.0.100", "192.168.1.50"]</c>
	///     </para>
	/// </remarks>
	public List<string> TrustedProxies { get; set; } = [];

	/// <summary>
	/// Gets or sets the list of network addresses (CIDR notation) that are considered trusted.
	/// Only relevant in <see cref="ForwardedHeaderMode.SelfManaged"/> mode.
	/// </summary>
	/// <remarks>
	///     <para>
	///     <strong>SelfManaged mode:</strong> Must contain at least one entry (this list OR <see cref="TrustedProxies"/>).
	///     Startup validation will fail if both lists are empty.
	///     </para>
	///     <para>
	///     Supports CIDR notation: <c>"192.168.1.0/24"</c> (subnet) or <c>"10.0.0.100/32"</c> (single IP).
	///     </para>
	///     <para>
	///     <strong>Cloud mode:</strong> This list is ignored (cloud platforms control headers).
	///     </para>
	...
	///     </para>
	///     <para>
	///     Example: <c>["10.0.0.0/8", "192.168.0.0/16"]</c>
	///     </para>
	/// </remarks>
	...
	/// </param>
	/// <param name="network">
	/// When the method returns <see langword="true"/>, contains the parsed <see cref="IPNetwork"/>;
	/// otherwise, the default value.
	/// </param>
	/// <param name="error">
	/// When the method returns <see langword="false"/>, contains a description of the validation error;
	...
	/// </returns>
	/// <remarks>
	/// The method checks that the input string is in the correct CIDR format, that the IP address is
...
public enum ForwardedHeaderMode
{
	...
	/// This is the default (secure-by-default).
	/// </summary>
	Disabled,

	/// <summary>
	/// Trust all forwarded headers from platform-managed reverse proxies.
	/// Use this when deploying to cloud platforms (Azure App Service, AWS Elastic Beanstalk, Google Cloud Run)
	/// where the platform controls the reverse proxy infrastructure.
	/// </summary>
	/// <remarks>
	/// Use this mode only when the application is guaranteed to receive traffic exclusively through a
	/// platform-managed reverse proxy (Azure, AWS, GCP). If direct public access is possible, prefer
	/// <see cref="SelfManaged"/> and configure trusted proxies or networks explicitly.
	/// </remarks>
	Cloud,

	/// <summary>
	/// Only trust forwarded headers from explicitly configured proxies and networks.
	/// Use this when running behind your own reverse proxy (Nginx, Traefik, Caddy) where you control
	/// the proxy IP addresses. Requires <see cref="ProxyHeadersOptions.TrustedProxies"/> or
	/// <see cref="ProxyHeadersOptions.TrustedNetworks"/> to be configured.
	/// </summary>
	SelfManaged
}

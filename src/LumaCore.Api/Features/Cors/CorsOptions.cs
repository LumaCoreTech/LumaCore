...
/// <remarks>
///     <para>
///     CORS allows web applications running on one origin (domain/port) to access resources
///     from a different origin. For security, browsers restrict cross-origin requests by default.
///     </para>
///     <para>
///     <strong>Important:</strong> CORS policies must be carefully configured to prevent unauthorized
...
public sealed class CorsOptions : IValidatableObject
{
	...
	/// <remarks>
	///     <para>
	///     When <see langword="false"/>, no CORS policy is applied and cross-origin requests will be
	///     blocked by browsers (default secure-by-default behavior).
	///     </para>
	///     <para>
	///     Set to <see langword="true"/> to enable CORS with the configured policy.
	///     </para>
	/// </remarks>
	...
	/// <remarks>
	///     <para>
	///     Specifies which origins (scheme + domain + port) are allowed to make cross-origin requests.
	///     </para>
	///     <para>
	...
	///     </list>
	///     <para>
	///     <strong>Security:</strong> Avoid using <c>"*"</c> in production. Specify exact origins instead.
	///     </para>
	/// </remarks>
	public List<string> AllowedOrigins { get; set; } = [];

	/// <summary>
	/// Gets or sets a value indicating whether credentials (cookies, authorization headers) should be allowed
	/// in cross-origin requests.
	/// </summary>
	/// <remarks>
	///     <para>
	///     When <see langword="true"/>, the server will include <c>Access-Control-Allow-Credentials: true</c>
	///     in responses, allowing browsers to include cookies and authorization headers in cross-origin requests.
	///     </para>
	///     <para>
	///     <strong>Security:</strong> Cannot be used with <c>AllowedOrigins = ["*"]</c>. You must specify
	...
	/// <remarks>
	///     <para>
	///     If empty, all HTTP methods are allowed.
	///     </para>
	///     <para>
	///         <strong>Example:</strong> <c>["GET", "POST", "PUT", "DELETE"]</c>
	///     </para>
	/// </remarks>
	...
	///     </para>
	///     <para>
	///         <strong>Example:</strong> <c>["Content-Type", "Authorization", "X-Custom-Header"]</c>
	///     </para>
	/// </remarks>
	...
	/// <remarks>
	///     <para>
	///     By default, only simple response headers (Cache-Control, Content-Language, Content-Type, etc.)
	///     are exposed to the client. To expose custom headers, list them here.
	///     </para>
	///     <para>
	...
	/// <remarks>
	///     <para>
	///     Preflight requests (OPTIONS) are sent by browsers before actual cross-origin requests.
	///     Setting a cache duration reduces the number of preflight requests.
	///     </para>
	///     <para>
	///     If <see langword="null"/>, no cache duration is specified (browser default applies).
	///     Typical values: <c>600</c> (10 minutes), <c>3600</c> (1 hour), <c>86400</c> (24 hours).
	///     </para>
	/// </remarks>
	[Range(0, int.MaxValue, ErrorMessage = "PreflightMaxAge must be greater than or equal to 0 when specified.")]

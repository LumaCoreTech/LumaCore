...
public static class ConversationQueries
{
	...
	/// <summary>
	/// Checks whether a participant belongs to a conversation.
	/// </summary>
	/// <remarks>
	/// Used for authorization checks on every conversation-scoped endpoint.
	/// </remarks>
	public static readonly Func<LumaCoreDbContext, ConversationId, ParticipantId, Task<bool>>
		IsParticipantInConversation = EF.CompileAsyncQuery((
				LumaCoreDbContext ctx,
				ConversationId    conversationId,
				ParticipantId     participantId) =>
			ctx.ConversationParticipants
				.AsNoTracking()
				.Any(cp => cp.ConversationId == conversationId && cp.ParticipantId == participantId));
}

...
> 💡 **Naming Convention:** Business API features are mounted on the central versioned `/api/v{version}` route group and map relative paths. For example, the *Auth* feature maps `/auth/*` which becomes `/api/v1/auth/*` at runtime. Infrastructure endpoints like `/health` are mapped directly to the application root for orchestrator compatibility, while the *Health* feature's API endpoint is versioned (`/api/v1/health/live`). The OpenAPI documentation at `/swagger` groups endpoints by feature tag and provides a version selector.

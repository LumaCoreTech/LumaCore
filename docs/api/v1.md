...
|--------|----------|-------------|
| 🟢 GET | [`/api/v1/admin/status`](#get-apiv1adminstatus) | Returns high-level status information about the API. |
...
returned by the `/api/v1/admin/status` endpoint.
...
by the `/api/v1/auth/whoami` endpoint.
...
Represents the response returned by the `/api/v1/auth/introspect` endpoint.
...
Represents the response returned by the `/api/v1/auth/whoami` endpoint.

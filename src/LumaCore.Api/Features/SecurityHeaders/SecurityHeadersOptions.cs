...
/// <remarks>
///     <para>
///     Security headers protect against common web vulnerabilities like clickjacking,
///     MIME-type sniffing, and cross-site scripting (XSS).
///     </para>
///     <para>
...
sealed class SecurityHeadersOptions : IValidatableObject
{
	...
	/// <remarks>
	///     <para>
	///     When <see langword="false"/>, no security headers are added.
	...
	///     </para>
	///     <para>
	///     <strong>Warning:</strong> Only enable HSTS when your site fully supports HTTPS.
	...
	/// <remarks>
	///     <para>
	///     Specifies how long browsers should remember to only use HTTPS.
	///     </para>
	///     <para>
	///     Common values:
	///     </para>
	///     <list type="bullet">
	///         <item><c>0</c> — Disable HSTS (clears browser's HSTS entry)</item>
	///         <item><c>86400</c> — 1 day (testing)</item>
	///         <item><c>2592000</c> — 30 days (initial rollout)</item>
	///         <item><c>31536000</c> — 1 year (production, default)</item>
	///     </list>
	/// </remarks>
	[Range(0, int.MaxValue, ErrorMessage = "HstsMaxAgeSeconds must be a non-negative value.")]
	...
	/// <remarks>
	///     <para>
	///     When <see langword="true"/>, the HSTS policy applies to all subdomains.
	...
	///     </list>
	///     <para>
	///     <strong>Note:</strong> <c>ALLOW-FROM</c> is deprecated and not supported.
	///     </para>
	/// </remarks>
	...
	///     Powerful protection against XSS attacks — even if malicious code is injected,
	///     the browser blocks loading resources from unauthorized sources.
	///     </para>
	///     <para>
	///     Set to <see langword="null"/> to disable the CSP header.
	///     </para>
	///     <para>
	...
	///         <item><c>script-src 'self' 'unsafe-eval'</c> — Scripts from same origin; <c>'unsafe-eval'</c> required for Blazor WebAssembly runtime</item>
	///         <item><c>style-src 'self' 'unsafe-inline'</c> — Styles from same origin; <c>'unsafe-inline'</c> required for Blazor's dynamic styling</item>
	///         <item><c>img-src 'self' data:</c> — Images from same origin plus inline <c>data:</c> URIs (common for small icons)</item>
	...
	///         <item><c>frame-ancestors 'none'</c> — Prevent embedding in iframes (same as <c>X-Frame-Options: DENY</c>)</item>
	///     </list>
	///     <para>
	///         <strong>Blazor WebAssembly requirements:</strong>
	///     </para>
	///     <para>
	///     Blazor WASM requires <c>'unsafe-eval'</c> for its .NET runtime and <c>'unsafe-inline'</c>
	///     for dynamic styles. These weaken CSP somewhat but are unavoidable for Blazor.
	///     A weak CSP is still better than no CSP — it blocks external script injection.
	///     </para>
	///     <para>
	...
	///     </list>
	/// </remarks>
	public string? ContentSecurityPolicy { get; set; } = "default-src 'self'; script-src 'self' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self'; connect-src 'self'; frame-ancestors 'none'";
